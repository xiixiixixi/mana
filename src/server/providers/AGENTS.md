...
- **Codex live usage**: prefer the installed Codex `app-server` method `account/rateLimits/read`; fall back to the legacy OAuth HTTP endpoint, then a clearly marked local rollout snapshot. Run the bundled Node with `--use-system-ca` and refresh macOS proxy state before live requests.
